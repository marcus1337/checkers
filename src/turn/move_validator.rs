...
pub fn can_step(turn: &Turn, from: Point, direction: Direction) -> bool {
    ...
    Action::get_step_actions(&board, from).iter().any(|action| action.get_direction() == direction)
}
...
pub fn can_jump(turn: &Turn, from: Point, direction: Direction) -> bool {
    ...
    Action::get_jump_actions(&board, from).iter().any(|action| action.get_direction() == direction)
}
